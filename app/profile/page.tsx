    ...
    <main className="bg-background min-h-screen pb-24">
      <div className="px-4 pt-8 pb-6 flex flex-col items-center">
        <div className="w-20 h-20 rounded-full bg-secondary flex items-center justify-center mb-4">
          <span className="text-2xl font-bold text-[#D63384]">{profile.name.split(' ').map(n => n[0]).join('')}</span>
        </div>
        <h2 className="text-xl font-bold text-foreground">{profile.name}</h2>
        <p className="text-sm text-muted-foreground mt-1">{profile.phone}</p>
        <button onClick={() => { setEditForm(profile); setIsEditing(true) }} className="mt-3 text-sm font-medium text-[#D63384]">
          Edit Profile
        </button>
      </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={editForm.name} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })}
                  className="w-full bg-background border border-border rounded-xl px-4 py-3 text-sm text-foreground focus:outline-none focus:ring-2 focus:ring-[#D63384]/30 focus:border-[#D63384]" />
              </div>
              <div>
              ...
              <button onClick={handleSave}
                className="w-full py-3 bg-[#D63384] text-white font-semibold rounded-xl hover:bg-[#AD1457] transition-colors flex items-center justify-center gap-2">
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-end sm:items-center justify-center">
          <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full max-w-sm p-6 shadow-xl">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-foreground">About The Wellness Hub</h3>
              <button onClick={() => setShowAbout(false)} className="w-8 h-8 rounded-full bg-background flex items-center justify-center">
                <X className="w-5 h-5 text-foreground" />
              </button>
            </div>
            <div className="space-y-3">
              <p className="text-sm text-foreground leading-relaxed">The Wellness Hub is a ladies-only wellness studio in 6th of October City, Giza, Egypt. Founded and led by trainer Enjy Gebril, we offer yoga, pilates, fitness, and dance classes in a supportive and empowering environment.</p>
              <p className="text-sm text-foreground leading-relaxed">With a 5.0-star Google rating, we are committed to helping every woman find her balance through movement and mindfulness.</p>
      ...
      <div className="px-4 space-y-4">
            ...
            {[
              ...
              <div key={i} className={`flex items-center justify-between px-4 py-3.5 ${i < 2 ? 'border-b border-border' : ''}`}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {item.badge && <span className="text-[10px] font-medium px-2 py-0.5 bg-[#4CAF50]/10 text-[#4CAF50] rounded-full">{item.badge}</span>}
                </div>
              </div>
            ))}
          </div>
        ...
        <div>
          ...
          <div className="bg-white border border-border rounded-2xl overflow-hidden shadow-sm">
            {[
              ...
              <div key={i} className={`flex items-center justify-between px-4 py-3.5 ${i < 2 ? 'border-b border-border' : ''}`}>
                <span className="text-sm text-foreground">{item.label}</span>
                <button onClick={() => setReminders(prev => ({ ...prev, [item.key]: !prev[item.key] }))}
        ...
        <div>
          ...
          <div className="bg-white border border-border rounded-2xl overflow-hidden shadow-sm">
            <a href="https://wa.me/201063751653" target="_blank" rel="noopener noreferrer" className="flex items-center w-full px-4 py-3.5 border-b border-border hover:bg-muted/30 transition-colors">
              <MessageCircle className="w-5 h-5 text-[#D63384] mr-3" />
            ...
            <a href="tel:+201063751653" className="flex items-center w-full px-4 py-3.5 border-b border-border hover:bg-muted/30 transition-colors">
              <Phone className="w-5 h-5 text-[#D63384] mr-3" />
            ...
            <a href="https://www.google.com/maps?q=29.978662,30.988026" target="_blank" rel="noopener noreferrer" className="flex items-center w-full px-4 py-3.5 border-b border-border hover:bg-muted/30 transition-colors">
              <MapPin className="w-5 h-5 text-[#D63384] mr-3" />
            ...
            <a href="https://www.instagram.com/yoga_together_forlife" target="_blank" rel="noopener noreferrer" className="flex items-center w-full px-4 py-3.5 border-b border-border hover:bg-muted/30 transition-colors">
              <Camera className="w-5 h-5 text-[#D63384] mr-3" />
            ...
            <button onClick={() => setShowAbout(true)} className="flex items-center w-full px-4 py-3.5 hover:bg-muted/30 transition-colors">
              <Info className="w-5 h-5 text-[#D63384] mr-3" />
              <span className="text-sm text-foreground flex-1 text-left">About The Wellness Hub</span>
              <ChevronRight className="w-4 h-4 text-muted-foreground/50" />
            </button>
          </div>
        </div>
        ...
        <div>
          <div className="bg-white border border-border rounded-2xl overflow-hidden shadow-sm">
            <button onClick={() => router.push('/login')} className="flex items-center w-full px-4 py-3.5 hover:bg-red-50 transition-colors">
